...
`cyy` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/cyy/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/cyy/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee cyy` to
...
python ./.trellis/scripts/task.py archive 00-join-cyy
```
