...
#### 3a. 配置 KV 模型
- `config` 表为 KV:`key` ∈ {`api_config`,`prompt_template`,`custom_default_template`,`weekly_map_template`,`weekly_reduce_template`,`weekly_default_map_template`,`weekly_default_reduce_template`,`export_dir`,`collect_config`,`auto_check_update`}。`auto_check_update` 为 bool,缺失时 `#[serde(default = "default_true")]` 回填 `true`(启动自动检查更新,见 `config.rs`)。
